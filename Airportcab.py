...
.progress-bar-inner {
    height: 100%;
    background: linear-gradient(90deg, #1B3A6B, #2E75B6, #F0A500);
    border-radius: 8px;
...
.rating-table th:first-child { text-align: left; padding-left: 16px; }
.rating-table td {
    padding: 10px 8px;
...
.rating-table tr:nth-child(even) td { background: var(--grey); }
...
div[data-testid="stButton"] > button:hover {
    background: linear-gradient(135deg, #1B3A6B, #2E75B6) !important;
    box-shadow: 0 6px 24px rgba(27,58,107,0.4) !important;
...
<div class="info-card">
    ...
    This survey takes less than <strong>5 minutes</strong> to complete. All responses are completely confidential.
</div>
""", unsafe_allow_html=True)
...
with col1:
    ...
    r1 = st.radio("", [
        ...
        "Airport Pick-up / Drop of a Guest",
        "Medical / Emergency",
        "Other"
...
with col4:
    ...
    r4 = st.radio("", [
        ...
        "Phone Call",
        "Walk-in / On-spot",
        "Corporate Account / Pre-arranged"
...
with col7:
    card_start()
    q_label(7, "How satisfied were you with fare transparency at the time of booking?")
    r7 = st.radio("", [
        "Completely Satisfied — Price was clear upfront",
        "Mostly Satisfied — Minor confusion",
        "Neutral",
        "Dissatisfied — Hidden charges noticed",
        "Very Dissatisfied — Pricing was unclear"
...
with col8:
    ...
    r8 = st.radio("", [
        "Yes, exactly on time",
        "Early — I preferred it",
        "Slightly late (less than 10 mins)",
        "Late (10–30 mins)",
        "Very late (more than 30 mins)"
...
r9 = st.radio("", [
    "Yes, I was informed well in advance",
    "Yes, but it was very last minute",
    "No, I found out on my own",
    "There was no delay / change"
...
# ══════════════════════════════════════════════
# SECTION C — IN-RIDE EXPERIENCE (RATING TABLE)
# ══════════════════════════════════════════════
...
ride_params = [
    "Vehicle Cleanliness",
    "Vehicle Comfort & Condition",
    "In-car Amenities (Water, Charger, etc.)",
    "AC / Temperature Control",
    "Driver's Behaviour & Courtesy",
    "Driver's Grooming & Appearance",
    "Safe & Smooth Driving",
    "Knowledge of Route / Navigation",
    "Music / Noise Level",
...
with col9:
    ...
    r11 = st.radio("", [
        "Yes, followed the best route",
        "Mostly yes, minor detour",
        "No, took an unnecessarily longer route",
        "I was unaware of the route"
...
with col11:
    ...
    r13 = st.radio("", [
        "Yes, exactly the right terminal",
        "Yes, but I had to walk some distance",
        "No, dropped at wrong terminal",
        "Not applicable"
    ], key="q13", label_visibility="collapsed")
...
with col12:
    ...
    r14 = st.radio("", [
        "Driver was waiting before I arrived",
        "Driver arrived within 5 minutes",
        "Had to wait more than 10 minutes",
        "Driver was hard to locate / communicate with",
        "Pick-up was confusing and poorly managed"
    ], key="q14", label_visibility="collapsed")
...
with col13:
    ...
    r15 = st.radio("", [
        "Yes, proactively and without being asked",
        "Yes, when I asked",
        "Partially",
        "No, did not assist",
        "I did not require assistance"
    ], key="q15", label_visibility="collapsed")
...
with col14:
    ...
    r16 = st.radio("", [
        "Reached well in advance — very comfortable",
        "Reached on time with reasonable buffer",
        "Just made it — very stressful",
        "Reached late — missed / nearly missed flight",
        "Not applicable — airport to city ride"
...
r17 = st.radio("", [
    "Yes, kept me updated proactively",
    "Communicated when I asked",
    "Did not communicate",
    "There was no delay"
...
# ══════════════════════════════════════════════
# SECTION E — DRIVER & SERVICE QUALITY
# ══════════════════════════════════════════════
...
with col15:
    ...
    ], key="q18", label_visibility="collapsed")
...
with col16:
    ...
    r19 = st.radio("", [
        ...
        "No, clearly unprofessional"
    ], key="q19", label_visibility="collapsed")
...
with col17:
    ...
    r20 = st.radio("", [
        "No, not at all",
        "Used hands-free / Bluetooth only",
        "Yes, briefly",
        "Yes, frequently — it was concerning"
    ], key="q20", label_visibility="collapsed")
...
with col18:
    ...
    r21 = st.radio("", [
        "No, professional throughout",
        "Hinted but did not pressure",
        "Yes, it was mildly uncomfortable",
        "Yes, it was very uncomfortable"
    ], key="q21", label_visibility="collapsed")
    responses["Q21_Tip_Pressure"] = r21
    card_end()


# ══════════════════════════════════════════════
# SECTION F — PRICING & VALUE
# ══════════════════════════════════════════════
...
with col19:
    ...
    r22 = st.radio("", [
        ...
        "Slight variation — acceptable",
        "Higher than quoted — unexplained",
        "Much higher than quoted"
    ], key="q22", label_visibility="collapsed")
...
with col20:
    ...
    r23 = st.radio("", [
        ...
        "Slightly overpriced",
        "Very overpriced"
    ], key="q23", label_visibility="collapsed")
...
r24 = st.radio("", [
    "Yes, multiple convenient options available",
    "Acceptable",
    "Limited options — minor inconvenience",
    "No, very limited — caused issues"
], key="q24", label_visibility="collapsed")
...
with col21:
    ...
    r25 = st.radio("", [
        ...
        "⭐⭐           Below Average (2/5)",
        "⭐              Poor (1/5)"
    ], key="q25", label_visibility="collapsed")
    responses["Q25_Overall_Rating"] = r25
    card_end()

with col22:
    card_start()
    q_label(26, "How likely are you to use our service again for your next airport trip?")
    r26 = st.radio("", [
        "Definitely will — my go-to service",
        "Very likely",
        "Possibly",
        "Unlikely",
        "Will not use again"
    ], key="q26", label_visibility="collapsed")
...
with col23:
    card_start()
    q_label(27, "Would you recommend our service to colleagues, friends, or family?")
    r27 = st.radio("", [
        "Yes, absolutely — would actively recommend",
        "Yes, if asked",
        "Neutral",
        "Probably not",
        "No, would not recommend"
    ], key="q27", label_visibility="collapsed")
    responses["Q27_Recommendation"] = r27
    card_end()

with col24:
    card_start()
    q_label(28, "Compared to other cab services for airport travel, how do we rank?")
    r28 = st.radio("", [
        "Best — significantly better",
        "Better than most",
        "About the same",
        "Below most",
        "Worst — significantly inferior"
    ], key="q28", label_visibility="collapsed")
...
# ══════════════════════════════════════════════
section_header("💬", "Section H — Open Feedback & Suggestions")

card_start()
q_label(29, "What did you like MOST about your experience today?")
r29 = st.text_area(
    "",
    placeholder="Tell us what stood out positively — the driver, the vehicle, the timing, the booking process...",
    height=100,
    key="q29",
    label_visibility="collapsed"
)
...
r30 = st.text_area(
    "",
    placeholder="Be candid — we want to know exactly what fell short so we can fix it...",
    height=100,
    key="q30",
    label_visibility="collapsed"
)
...
card_start()
q_label(31, "What ONE improvement would make the biggest difference for airport travellers?")
r31 = st.text_area(
    "",
    placeholder="Your single most impactful suggestion — app feature, driver training, pricing, routing...",
    height=100,
    key="q31",
    label_visibility="collapsed"
)
...
card_start()
q_label(32, "Any additional comments, compliments, or suggestions for our team?")
r32 = st.text_area(
    "",
    placeholder="Anything else you'd like us to know — a compliment for a great driver, a specific incident, a broader thought...",
    height=100,
    key="q32",
    label_visibility="collapsed"
)
...
answered = sum([
    ...
    bool(r7), bool(r8), bool(r9), bool(r11), bool(r12), bool(r13),
    bool(r14), bool(r15), bool(r16), bool(r17), bool(r18), bool(r19),
    bool(r20), bool(r21), bool(r22), bool(r23), bool(r24), bool(r25),
    bool(r26), bool(r27), bool(r28)
])
total_mcq = 27
pct = int((answered / total_mcq) * 100)
